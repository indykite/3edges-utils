 ...
 */
export class MetricsMgr
{
    private static _instance: MetricsMgr;
    private readonly _registry: prom.Registry;
    private _Counters : Map<string,prom.Counter<string>>;
    private readonly _CounterValues : Map<string,number>;
    private _Gauges : Map<string,prom.Gauge<string>>;
    private readonly _GaugeValues : Map<string,number>;
    private _Histograms : Map<string,prom.Histogram<string>>;
     ...
     */
    public createCounter (cName: string, cHelp: string) {
        // Counter values
     ...
     */
    public createCounterWithLabels (cName: string, cHelp: string, cLabels: string[]) {
        // Counter values
        this._CounterValues.set(cName,0);
     ...
     */
    public addToCounter (cName: string, val: number, label?: object) {
     ...
     */
    public getCounter(cName: string): Counter<string> {
     ...
     * @protected
     */
    public getCounterValues (): Map<string,number> {
     ...
     * @protected
     */
    public getCounterValue (cName: string): number {
     ...
     */
    public createGauge (gName: string, gHelp: string) {
        // Counter values
     ...
     */
    public createGaugeWithLabels (gName: string, gHelp: string, gLabels: string[]) {
        // Counter values
        this._GaugeValues.set(gName,0);
     ...
     * in the form "{key: value}" - e.g., "{ code: 200 }"
     */
    public addToGauge (gName: string, val: number, label?: object) {
        // eslint-disable-next-line
     ...
     * in the form "{key: value}" - e.g., "{ code: 200 }"
     */
    public decGauge (gName: string, val: number, label?: object) {
        // eslint-disable-next-line
        let c = this._Gauges.get(gName);
        if (!label)
            c.dec(val);
     ...
     * @param cName the name of the counter to return
     */
    public getGauge(gName: string): Gauge<string> {
     ...
     * @protected
     */
    public getGaugeValues (): Map<string,number> {
     ...
     * @protected
     */
    public getGaugeValue (gName: string): number {
     ...
     */
    public startTimer (tName: string) {
     ...
     */
    public createHistogram (hName: string, hHelp: string, hLabels?: string[], hBuckets?: number[]) {
        ...
        // Optional Buckets
        if (hBuckets) {
            cfg ={...cfg, buckets: hBuckets};
        }
        // Optional Labels
        if (hLabels) {
            cfg ={...cfg, labelNames: hLabels};
        }
        // Configure Histogram
     ...
     */
    public observe (hName: string, hValue: number, hLabels?: string) {
        if (hLabels) {
     ...
     */
    public getHistogram (hName: string): Histogram<string> {
     ...
     */
    public startHistoTimer (hName: string, hLabels?: string) {
        // start timer: returns a stop timer `end` function
        if (hLabels) {
     ...
     */
    public defaultMetrics (prefix: string) {
